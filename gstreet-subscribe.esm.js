...
Caused by: ${(p = u?.name) !== null && p !== void 0 ? p : "Error"}: ${_}`, w && (o += ` (${w})`), u?.stack && (o += `
${u.stack}`);
      } else {
        var g;
...
class nr {
  ...
  _encodeUserBroadcastPush(e, t, s) {
    ...
    p.setUint8(g++, this.KINDS.userBroadcastPush), p.setUint8(g++, l.length), p.setUint8(g++, o.length), p.setUint8(g++, a.length), p.setUint8(g++, c.length), p.setUint8(g++, f.length), p.setUint8(g++, t), Array.from(l, (_) => p.setUint8(g++, _.charCodeAt(0))), Array.from(o, (_) => p.setUint8(g++, _.charCodeAt(0))), Array.from(a, (_) => p.setUint8(g++, _.charCodeAt(0))), Array.from(c, (_) => p.setUint8(g++, _.charCodeAt(0))), Array.from(f, (_) => p.setUint8(g++, _.charCodeAt(0)));
    ...
    if (this.socket.isConnected() || this.socket.connect(), this.state == P.closed) {
      ...
      this.socket.accessTokenValue && (f.access_token = this.socket.accessTokenValue), this._onError((h) => e?.(q.CHANNEL_ERROR, h)), this._onClose(() => e?.(q.CLOSED)), this.updateJoinPayload(Object.assign({ config: d }, f)), this.joinedOnce = !0, this._rejoin(t), this.joinPush.receive("ok", async ({ postgres_changes: h }) => {
        ...
        if (this.socket._isManualToken() || this.socket.setAuth(), h === void 0) {
          ...
          const g = this.bindings.postgres_changes, y = (p = g?.length) !== null && p !== void 0 ? p : 0, _ = [];
          for (let w = 0; w < y; w++) {
    ...
    ["insert", "update", "delete"].includes(a) ? (n = this.bindings.postgres_changes) === null || n === void 0 || n.filter((h) => {
      ...
      if (["broadcast", "presence", "postgres_changes"].includes(a))
        if ("id" in h) {
          ...
          const k = (w = (_ = h?.filter) === null || _ === void 0 ? void 0 : _.event) === null || w === void 0 ? void 0 : w.toLocaleLowerCase();
      ...
      if (typeof d == "object" && "ids" in d) {
        const p = d.data, { schema: g, table: y, commit_timestamp: _, type: w, errors: v } = p;
...
class ve {
  ...
  constructor(e) {
    ...
    }, this.persistSession ? (i.storage ? this.storage = i.storage : Ut() ? this.storage = globalThis.localStorage : (this.memoryStorage = {}, this.storage = mt(this.memoryStorage)), i.userStorage && (this.userStorage = i.userStorage)) : (this.memoryStorage = {}, this.storage = mt(this.memoryStorage)), j() && globalThis.BroadcastChannel && this.persistSession && this.storageKey) {
      try {
...
const vn = "https://qxozqgpuzthemsfotmvo.supabase.co", _n = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF4b3pxZ3B1enRoZW1zZm90bXZvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA2Nzg2NTUsImV4cCI6MjA4NjI1NDY1NX0.N_10hGPsTjO4aH5DkqVd3EkE14QLpuOQoENgff0uTbE", kt = gn(vn, _n), mn = [
  "community",
  "bg",
  "surface",
  "primary",
...
class wn extends HTMLElement {
  ...
  constructor() {
    super(), this._shadowRoot = this.attachShadow({ mode: "open" }), this._state = "idle", this._errorMsg = "", this._email = "", this._name = "";
  }
  connectedCallback() {
    this._render();
  }
  ...
  async _handleSubmit(e) {
    e.preventDefault();
    const t = this._shadowRoot.querySelector("form"), s = t.querySelector('input[type="email"]'), n = t.querySelector('input[name="name"]'), i = s.value.trim(), a = n ? n.value.trim() : null;
    if (!i) return;
    const o = this.getAttribute("community");
    if (!o) {
    ...
    try {
      const { data: l, error: c } = await kt.from("communities").select("id").eq("slug", o).single();
      if (c || !l)
        throw new Error("Community not found");
      const { error: u } = await kt.from("subscribers").insert({
        community_id: l.id,
        email: i,
        name: a || null,
      ...
      if (u) {
        if (u.code === "23505") {
          this._state = "duplicate", this._render(), this._dispatch("subscribe:error", { error: "Already subscribed" });
          return;
        }
        throw u;
      }
      this._state = "success", this._render(), this._dispatch("subscribe:success", { email: i, community: o });
    } catch (l) {
      this._state = "error", this._errorMsg = l.message || "Something went wrong", this._render(), this._dispatch("subscribe:error", { error: l.message });
    }
  }
  _render() {
    const e = this._getTheme(), t = this.getAttribute("heading") || "Stay in the loop", s = this.getAttribute("subheading") || "", n = this.getAttribute("button-text") || "Subscribe", i = this.hasAttribute("show-name");
    ...
    if (this._state === "duplicate") {
      this._shadowRoot.innerHTML = `
        <style>${this._getStyles(e)}</style>
        <div class="container">
          <p class="duplicate-text">You're already subscribed!</p>
        </div>
      `;
      return;
    }
    const o = this._state === "submitting";
    this._shadowRoot.innerHTML = `
      <style>${this._getStyles(e)}</style>
      <div class="container">
        ...
        <form>
          ...
          <input
            ...
            autocomplete="email"
          />
          <button type="submit" class="btn" ${o ? "disabled" : ""}>
            ${o ? "Subscribing..." : this._escapeHtml(n)}
          </button>
        </form>
        ${this._state === "error" ? `<p class="error-text">${this._escapeHtml(this._errorMsg)}</p>` : ""}
      </div>
    `;
    const l = this._shadowRoot.querySelector("form");
    l && l.addEventListener("submit", (c) => this._handleSubmit(c));
  }
  _escapeHtml(e) {
...
customElements.get("gstreet-subscribe") || customElements.define("gstreet-subscribe", wn);
export {
  wn as GstreetSubscribeElement
};
