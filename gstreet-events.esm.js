  ...
  invoke(e) {
    return qt(this, arguments, void 0, function* (t, s = {}) {
      ...
      try {
        ...
        let v = u;
        f && (a = new AbortController(), i = setTimeout(() => a.abort(), f), u ? (v = a.signal, u.addEventListener("abort", () => a.abort())) : v = a.signal);
        ...
        if (!_.ok)
          throw new tt(_);
        let y = ((n = _.headers.get("Content-Type")) !== null && n !== void 0 ? n : "text/plain").split(";")[0].trim(), S;
        return y === "application/json" ? S = yield _.json() : y === "application/octet-stream" || y === "application/pdf" ? S = yield _.blob() : y === "text/event-stream" ? S = _ : y === "multipart/form-data" ? S = yield _.formData() : S = yield _.text(), { data: S, error: null, response: _ };
      } catch (o) {
        return {
          data: null,
          error: o,
  ...
  then(r, e) {
    ...
    let n = s(this.url.toString(), {
      ...
      if (i.ok) {
        ...
        const g = (f = t.headers.get("Prefer")) === null || f === void 0 ? void 0 : f.match(/count=(exact|planned|estimated)/), v = (d = i.headers.get("content-range")) === null || d === void 0 ? void 0 : d.split("/");
        g && v && v.length > 1 && (l = parseInt(v[1])), t.isMaybeSingle && t.method === "GET" && Array.isArray(o) && (o.length > 1 ? (a = {
          code: "PGRST116",
...
${u.stack}`);
      ...
      return i?.name === "AbortError" || i?.code === "ABORT_ERR" ? (c = "", l = "Request was aborted (timeout or manual cancellation)", v > this.urlLengthLimit && (l += `. Note: Your request URL is ${v} characters, which may exceed server limits. If selecting many fields, consider using views. If filtering with large arrays (e.g., .in('id', [many IDs])), consider using an RPC function to pass values server-side.`)) : (u?.name === "HeadersOverflowError" || u?.code === "UND_ERR_HEADERS_OVERFLOW") && (c = "", l = "HTTP headers exceeded server limits (typically 16KB)", v > this.urlLengthLimit && (l += `. Your request URL is ${v} characters. If selecting many fields, consider using views. If filtering with large arrays (e.g., .in('id', [200+ IDs])), consider using an RPC function instead.`)), {
        error: {
...
const Zt = "2.98.0", er = `realtime-js/${Zt}`, tr = "1.0.0", Tt = "2.0.0", nt = Tt, We = 1e4, rr = 1e3, sr = 100;
var W;
(function(r) {
...
class nr {
  ...
  _encodeUserBroadcastPush(e, t, s) {
    ...
    return v.set(new Uint8Array(h), 0), v.set(new Uint8Array(s), h.byteLength), v.buffer;
  }
  decode(e, t) {
...
const it = (r, e, t = {}) => {
  ...
  return i && !s.includes(i) ? At(i, a) : He(a);
}, At = (r, e) => {
  if (r.charAt(0) === "_") {
...
class ue {
  ...
  static syncState(e, t, s, n) {
    ...
    return this.map(i, (c, u) => {
      ...
      const f = i[c];
      if (f) {
        const d = u.map((v) => v.presence_ref), h = f.map((v) => v.presence_ref), p = u.filter((v) => h.indexOf(v.presence_ref) < 0), g = f.filter((v) => d.indexOf(v.presence_ref) < 0);
...
class ie {
    ...
    if (this.socket.isConnected() || this.socket.connect(), this.state == P.closed) {
      ...
      this.socket.accessTokenValue && (f.access_token = this.socket.accessTokenValue), this._onError((h) => e?.(q.CHANNEL_ERROR, h)), this._onClose(() => e?.(q.CLOSED)), this.updateJoinPayload(Object.assign({ config: d }, f)), this.joinedOnce = !0, this._rejoin(t), this.joinPush.receive("ok", async ({ postgres_changes: h }) => {
        ...
        if (this.socket._isManualToken() || this.socket.setAuth(), h === void 0) {
          ...
          const g = this.bindings.postgres_changes, v = (p = g?.length) !== null && p !== void 0 ? p : 0, _ = [];
          for (let w = 0; w < v; w++) {
            const y = g[w], { filter: { event: S, schema: C, table: E, filter: R } } = y, F = h && h[w];
            if (F && F.event === S && ie.isFilterValueEqual(F.schema, C) && ie.isFilterValueEqual(F.table, E) && ie.isFilterValueEqual(F.filter, R))
              _.push(Object.assign(Object.assign({}, y), { id: F.id }));
            else {
  ...
  _trigger(e, t, s) {
    ...
    ["insert", "update", "delete"].includes(a) ? (n = this.bindings.postgres_changes) === null || n === void 0 || n.filter((h) => {
      var p, g, v;
      return ((p = h.filter) === null || p === void 0 ? void 0 : p.event) === "*" || ((v = (g = h.filter) === null || g === void 0 ? void 0 : g.event) === null || v === void 0 ? void 0 : v.toLocaleLowerCase()) === a;
    }).map((h) => h.callback(d, s)) : (i = this.bindings[a]) === null || i === void 0 || i.filter((h) => {
      var p, g, v, _, w, y;
      if (["broadcast", "presence", "postgres_changes"].includes(a))
        if ("id" in h) {
          const S = h.id, C = (p = h.filter) === null || p === void 0 ? void 0 : p.event;
          return S && ((g = t.ids) === null || g === void 0 ? void 0 : g.includes(S)) && (C === "*" || C?.toLocaleLowerCase() === ((v = t.data) === null || v === void 0 ? void 0 : v.type.toLocaleLowerCase()));
        } else {
      ...
      if (typeof d == "object" && "ids" in d) {
        const p = d.data, { schema: g, table: v, commit_timestamp: _, type: w, errors: y } = p;
  ...
  _getPayloadRecords(e) {
    ...
    return (e.type === "INSERT" || e.type === "UPDATE") && (t.new = it(e.columns, e.record)), (e.type === "UPDATE" || e.type === "DELETE") && (t.old = it(e.columns, e.old_record)), t;
  }
}
const Ue = () => {
}, be = {
...
class pr {
  ...
  constructor(e, t) {
    var s;
    if (this.accessTokenValue = null, this.apiKey = null, this._manuallySetToken = !1, this.channels = new Array(), this.endPoint = "", this.httpEndpoint = "", this.headers = {}, this.params = {}, this.timeout = We, this.transport = null, this.heartbeatIntervalMs = be.HEARTBEAT_INTERVAL, this.heartbeatTimer = void 0, this.pendingHeartbeatRef = null, this.heartbeatCallback = Ue, this.ref = 0, this.reconnectTimer = null, this.vsn = nt, this.logger = Ue, this.conn = null, this.sendBuffer = [], this.serializer = new nr(), this.stateChangeCallbacks = {
      open: [],
      close: [],
      error: [],
      message: []
...
Option 2: Install and provide the "ws" package:
  ...
  _initializeOptions(e) {
    ...
    switch (this.transport = (t = e?.transport) !== null && t !== void 0 ? t : null, this.timeout = (s = e?.timeout) !== null && s !== void 0 ? s : We, this.heartbeatIntervalMs = (n = e?.heartbeatIntervalMs) !== null && n !== void 0 ? n : be.HEARTBEAT_INTERVAL, this.worker = (i = e?.worker) !== null && i !== void 0 ? i : !1, this.accessToken = (a = e?.accessToken) !== null && a !== void 0 ? a : null, this.heartbeatCallback = (o = e?.heartbeatCallback) !== null && o !== void 0 ? o : Ue, this.vsn = (l = e?.vsn) !== null && l !== void 0 ? l : nt, e?.params && (this.params = e.params), e?.logger && (this.logger = e.logger), (e?.logLevel || e?.log_level) && (this.logLevel = e.logLevel || e.log_level, this.params = Object.assign(Object.assign({}, this.params), { log_level: this.logLevel })), this.reconnectAfterMs = (c = e?.reconnectAfterMs) !== null && c !== void 0 ? c : ((p) => hr[p - 1] || dr), this.vsn) {
      case tr:
...
function yr(r) {
  ...
  return {
    async request({
      ...
      const o = gr(r.baseUrl, s, n), l = await vr(r.auth), c = await e(o, {
        method: t,
        headers: {
  ...
  */
  async uploadOrUpdate(r, e, t, s) {
    ...
    return n.handleOperation(async () => {
      ...
      const c = n._removeEmptyFolders(e), u = n._getFinalPath(c), f = await (r == "PUT" ? ze : L)(n.fetch, `${n.url}/object/${u}`, i, b({ headers: o }, a?.duplex ? { duplex: a.duplex } : {}));
      return {
        path: c,
        id: f.Id,
        fullPath: f.Key
  ...
  */
  async uploadToSignedUrl(r, e, t, s) {
    ...
    return o.searchParams.set("token", e), n.handleOperation(async () => {
      let l;
      const c = b({ upsert: ut.upsert }, s), u = b(b({}, n.headers), { "x-upsert": String(c.upsert) });
      return typeof Blob < "u" && t instanceof Blob ? (l = new FormData(), l.append("cacheControl", c.cacheControl), l.append("", t)) : typeof FormData < "u" && t instanceof FormData ? (l = t, l.append("cacheControl", c.cacheControl)) : (l = t, u["cache-control"] = `max-age=${c.cacheControl}`, u["content-type"] = c.contentType), {
        path: i,
        fullPath: (await ze(n.fetch, o.toString(), l, { headers: u })).Key
      };
    });
  }
  /**
...
const $t = "2.98.0", se = 30 * 1e3, Je = 3, Le = Je * se, Hr = "http://localhost:9999", Vr = "supabase.auth.token", zr = { "X-Client-Info": `gotrue-js/${$t}` }, Ge = "X-Supabase-Api-Version", jt = {
  "2024-01-01": {
...
const Ae = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_".split(""), ft = ` 	
\r=`.split(""), es = (() => {
...
async function k(r, e, t, s) {
  ...
  i[Ge] || (i[Ge] = jt["2024-01-01"].name), s?.jwt && (i.Authorization = `Bearer ${s.jwt}`);
...
class Js {
  ...
  async _authenticate({ factorId: e, webauthn: { rpId: t = typeof window < "u" ? window.location.hostname : void 0, rpOrigins: s = typeof window < "u" ? [window.location.origin] : void 0, signal: n } = {} }, i) {
    if (!t)
      return {
        data: null,
        error: new ve("rpId is required for WebAuthn authentication")
      };
    try {
      if (!bt())
        return {
  ...
  async _register({ friendlyName: e, webauthn: { rpId: t = typeof window < "u" ? window.location.hostname : void 0, rpOrigins: s = typeof window < "u" ? [window.location.origin] : void 0, signal: n } = {} }, i) {
    if (!t)
      return {
        data: null,
        error: new ve("rpId is required for WebAuthn registration")
      };
    try {
      if (!bt())
        return {
...
class ye {
  ...
  constructor(e) {
    ...
    if (this.storageKey = i.storageKey, this.instanceID = (t = ye.nextInstanceID[this.storageKey]) !== null && t !== void 0 ? t : 0, ye.nextInstanceID[this.storageKey] = this.instanceID + 1, this.logDebugMessages = !!i.debug, typeof i.debug == "function" && (this.logger = i.debug), this.instanceID > 0 && $()) {
    ...
    if (this.persistSession = i.persistSession, this.autoRefreshToken = i.autoRefreshToken, this.admin = new As({
      ...
      headers: i.headers,
      fetch: i.fetch
    }), this.url = i.url, this.headers = i.headers, this.fetch = Lt(i.fetch), this.lock = i.lock || kt, this.detectSessionInUrl = i.detectSessionInUrl, this.flowType = i.flowType, this.hasCustomAuthorizationHeader = i.hasCustomAuthorizationHeader, this.throwOnError = i.throwOnError, this.lockAcquireTimeout = i.lockAcquireTimeout, i.lock ? this.lock = i.lock : this.persistSession && $() && (!((s = globalThis?.navigator) === null || s === void 0) && s.locks) ? this.lock = Ps : this.lock = kt, this.jwks || (this.jwks = { keys: [] }, this.jwks_cached_at = Number.MIN_SAFE_INTEGER), this.mfa = {
  ...
  async signUp(e) {
    ...
    try {
      ...
      if ("email" in e) {
        ...
        let h = null, p = null;
        this.flowType === "pkce" && ([h, p] = await Z(this.storage, this.storageKey)), i = await k(this.fetch, "POST", `${this.url}/signup`, {
          headers: this.headers,
  ...
  async signInWithEthereum(e) {
    ...
    if ("message" in e)
      ...
      const S = new URL((t = w?.url) !== null && t !== void 0 ? t : window.location.href), C = await y.request({
      ...
      h = js(F), p = await y.request({
        method: "personal_sign",
        params: [$s(h), E]
      });
    }
    try {
      const { data: g, error: v } = await k(this.fetch, "POST", `${this.url}/token?grant_type=web3`, {
        headers: this.headers,
        body: Object.assign({
          chain: "ethereum",
          message: h,
  ...
  async signInWithSolana(e) {
    ...
    if ("message" in e)
      ...
      if ($())
        ...
        if (typeof _ != "object" || !y?.url)
          throw new Error("@supabase/auth-js: Both wallet and url must be specified in non-browser environments.");
        S = _;
      }
      const C = new URL((t = y?.url) !== null && t !== void 0 ? t : window.location.href);
      if ("signIn" in S && S.signIn) {
        const E = await S.signIn(Object.assign(Object.assign(Object.assign({ issuedAt: (/* @__PURE__ */ new Date()).toISOString() }, y?.signInWithSolana), {
        ...
        p = [
          ...
          "Version: 1",
          `URI: ${C.href}`,
          `Issued At: ${(n = (s = y?.signInWithSolana) === null || s === void 0 ? void 0 : s.issuedAt) !== null && n !== void 0 ? n : (/* @__PURE__ */ new Date()).toISOString()}`,
          ...
          ...!((f = (u = y?.signInWithSolana) === null || u === void 0 ? void 0 : u.resources) === null || f === void 0) && f.length ? [
            "Resources",
...
`);
    ...
    try {
      const { data: v, error: _ } = await k(this.fetch, "POST", `${this.url}/token?grant_type=web3`, {
        headers: this.headers,
  ...
  async _getAuthenticatorAssuranceLevel(e) {
    ...
    if (e)
      try {
        ...
        ((s = (t = v?.factors) === null || t === void 0 ? void 0 : t.filter((S) => S.status === "verified")) !== null && s !== void 0 ? s : []).length > 0 && (g = "aal2");
        const y = h.amr || [];
...
var fn = class extends Ys {
  ...
  */
  ...
  _initRealtimeClient(r) {
    return new pr(this.realtimeUrl.href, O(O({}, r), {}, { params: O(O({}, { apikey: this.supabaseKey }), r?.params) }));
  }
...
const yn = "https://qxozqgpuzthemsfotmvo.supabase.co", _n = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF4b3pxZ3B1enRoZW1zZm90bXZvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA2Nzg2NTUsImV4cCI6MjA4NjI1NDY1NX0.N_10hGPsTjO4aH5DkqVd3EkE14QLpuOQoENgff0uTbE", Me = gn(yn, _n), mn = [
  "community",
  "bg",
  "surface",
  "primary",
...
class bn extends HTMLElement {
  ...
  constructor() {
    super(), this._shadowRoot = this.attachShadow({ mode: "open" }), this._state = "loading", this._errorMsg = "", this._upcoming = [], this._past = [];
  }
  connectedCallback() {
    this._render(), this._fetchEvents();
  }
  attributeChangedCallback(e, t, s) {
    t !== s && this._fetchEvents();
  }
  _dispatch(e, t = {}) {
  ...
  async _fetchEvents() {
    ...
    try {
      const { data: t, error: s } = await Me.from("communities").select("id").eq("slug", e).single();
      if (s || !t)
        throw new Error("Community not found");
      const n = parseInt(this.getAttribute("max") || "5", 10) || 5, i = this.hasAttribute("show-past"), a = (/* @__PURE__ */ new Date()).toISOString(), { data: o, error: l } = await Me.from("community_events").select("*").eq("community_id", t.id).gte("starts_at", a).order("starts_at", { ascending: !0 }).limit(n);
      if (l) throw l;
      if (this._upcoming = o || [], i) {
        const { data: c, error: u } = await Me.from("community_events").select("*").eq("community_id", t.id).lt("starts_at", a).order("starts_at", { ascending: !1 }).limit(n);
        if (u) throw u;
        this._past = c || [];
      } else
        this._past = [];
      this._state = "loaded", this._render(), this._dispatch("events:loaded", { count: this._upcoming.length + this._past.length });
    } catch (t) {
      this._state = "error", this._errorMsg = t.message || "Something went wrong", this._render();
    }
  }
  _formatTime(e) {
    return new Date(e).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" });
  }
  _formatTimeRange(e, t) {
    const s = this._formatTime(e);
    return t ? `${s} – ${this._formatTime(t)}` : s;
  }
  _renderEventCard(e, t) {
    const s = new Date(e.starts_at), n = wn[s.getMonth()], i = s.getDate(), a = e.description ? e.description.length > 120 ? e.description.slice(0, 120) + "..." : e.description : "";
    return `
      <div class="event-card ${t ? "past" : ""}">
        <div class="date-badge">
          <div class="date-month">${n}</div>
          <div class="date-day">${i}</div>
        </div>
        <div class="event-details">
          <div class="event-title">${this._escapeHtml(e.title)}</div>
          <div class="event-time">${this._formatTimeRange(e.starts_at, e.ends_at)}</div>
          ${e.location ? `<div class="event-location">📍 ${this._escapeHtml(e.location)}</div>` : ""}
          ${a ? `<div class="event-desc">${this._escapeHtml(a)}</div>` : ""}
          ${e.event_url ? `<a class="event-link" href="${this._escapeHtml(e.event_url)}" target="_blank" rel="noopener noreferrer">Learn More →</a>` : ""}
        </div>
      </div>
    `;
  }
  _render() {
    const e = this._getTheme(), t = this.getAttribute("heading") || "Upcoming Events";
    ...
    const n = this._upcoming.length > 0, i = this._past.length > 0;
    if (!n && !i) {
      this._shadowRoot.innerHTML = `
        <style>${this._getStyles(e)}</style>
        <div class="container">
      ...
      <div class="container">
        ...
        ${n ? `
          <div class="events-list">
            ${this._upcoming.map((a) => this._renderEventCard(a, !1)).join("")}
          </div>
        ` : ""}
        ${i ? `
          <div class="past-divider">Past Events</div>
          <div class="events-list">
            ${this._past.map((a) => this._renderEventCard(a, !0)).join("")}
          </div>
        ` : ""}
      </div>
    `;
...
customElements.get("gstreet-events") || customElements.define("gstreet-events", bn);
export {
  bn as GstreetEventsElement
};
